...
############################################################################################
#Section 5 - Binary models for testing taxon-specific bias
...
print(mydf)
#write_xlsx(mydf, "CO1_model_phylum.xlsx") #outputs sd.error/robust error/confidence intervals/predicted means
#There were no issues with the calculation of robust errors
...
# Display the modified data frame
print(mydf)
#write_xlsx(mydf, "CO1_model_title.xlsx") #outputs sd.error/robust error/confidence intervals/predicted means
#No issues with the calculation of robust standard errors
...
#Test for over dispersion and normality of data
...
outliers <- which(cook_distance > 4 / nrow(data_co1_model))  # Using a common threshold of 4/n
...
##there were some separation issues - mild and glm model converged and robust standard errors were calculated


###model - 16s 
subset_data <- data_16_model[, c("title", "eDNA", "Phylum")]
...
##Robust standard errors could not be calculated. Cannot deal with heteroscedasticity. Unreliable model. Largely due to sample size and separation issues. Sample size cannot be addressed.  

#Check for separation
freq_table <- table(subset_data$eDNA)
print(freq_table) #there is separation. But no convergence issues.

#Check clustering
residuals_df <- data.frame(Residuals = residuals(model, type = "pearson"), Phylum = data_16_model$Phylum)
boxplot(Residuals ~ Phylum, data = residuals_df, xlab = "Phylum", ylab = "Residuals", main = "Residuals by Phylum")
...
#Leverage plot
plot(hatvalues(model), residuals(model)^2, xlab = "Leverage", ylab = "Residuals Squared", main = "Leverage vs. Residuals Squared")
bptest(model) #there is heterosk in model potentially the result of small dataset and separation issues
...
cook_distance <- influence_obj$cooks.distance
# Identify observations with high Cook's distance
outliers <- which(cook_distance > 4 / nrow(data_16_model))  # Using a common threshold of 4/n
# Print or inspect the indices of the identified outliers
print(outliers) #NO outliers

#AICc selection
options(na.action = "na.fail")
dredge1 <- dredge(model, rank = "AICc") #When accounting for separation issues the best supported model is the null model
dredge1 #null model is much worse
#Unreliable model. Clear issues with sample size and separation issues. Robust standard errors cannot be calculated.
...
# Conduct hypothesis tests using the robust standard errors
summary(model, robust = TRUE)
##Robust standard errors could be calculated.
...
#Test for overdispersion and normality of data
testDispersion(model)
model_test<- simulateResiduals(model, refit=T)
testOverdispersion(model_test)
plot(model_test) #NOT OVERDISPERSED + normality of data tested
...
#Check for separation - there are seperation issues
freq_table <- table(data_18_model$eDNA)
print(freq_table) #there is separation - low values.
...
options(na.action = "na.fail")
dredge1 <- dredge(model, rank = "AICc") #AIC values all within value of 6
dredge1 #null model is the best.
